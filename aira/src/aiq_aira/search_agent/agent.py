...
class SearchGraphState(BaseModel):
...
class SearchAgentGraph(DualNodeAgent):
    """Configurable LangGraph Search ReAct Agent. A Search ReAct Agent performs reasoning inbetween tool calls, and utilizes the tool
    ...
    async def agent_node(self, state: SearchGraphState):
        try:
            logger.debug("%s Starting the Search ReAct Agent Node", AGENT_LOG_PREFIX)
            ...
            for attempt in range(1, self.max_tries + 1):
                ...
                try:
                    ...
                    logger.info("%s Retrying Search ReAct Agent, including output parsing Observation", AGENT_LOG_PREFIX)
    ...
    async def conditional_edge(self, state: SearchGraphState):
        try:
            logger.debug("%s Starting the Search ReAct Conditional Edge", AGENT_LOG_PREFIX)
    ...
    async def tool_node(self, state: SearchGraphState):
        try:
            ...
            if not requested_tool:
                ...
                logger.warning(
                    "%s Search Agent wants to call tool %s. In the Search Agent's configuration within the config file,"
    ...
    async def build_graph(self):
        try:
            await super()._build_graph(state_schema=SearchGraphState)
            logger.debug("%s ReAct Graph built and compiled successfully", AGENT_LOG_PREFIX)
            return self.graph
        except Exception as ex:
            logger.exception("%s Failed to build Search ReAct Graph: %s", AGENT_LOG_PREFIX, ex, exc_info=ex)
            raise ex

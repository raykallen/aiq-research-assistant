...
class SearchAgentWorkflowConfig(FunctionBaseConfig, name="search_agent"):
    """
...
@register_function(config_type=SearchAgentWorkflowConfig, framework_wrappers=[LLMFrameworkEnum.LANGCHAIN])
async def search_agent_workflow(config: SearchAgentWorkflowConfig, builder: Builder):
    ...
    from aiq_aira.search_agent.prompt import USER_PROMPT

    from aiq_aira.search_agent.agent import SearchAgentGraph
    from aiq_aira.search_agent.agent import SearchGraphState
    from aiq_aira.search_agent.prompt import search_agent_prompt
    ...
    if config.system_prompt:
        ...
        valid_prompt = SearchAgentGraph.validate_system_prompt(config.system_prompt)
        if not valid_prompt:
    ...
    graph: CompiledGraph = await SearchAgentGraph(llm=llm,
                                                 prompt=prompt,
                                                 tools=tools,
    ...
    async def _response_fn(input_message: AIQChatRequest) -> AIQChatResponse:
        try:
            ...
            state = SearchGraphState(messages=messages)

...
async def web_research(
    ...
    """
    Gathers information by calling a search agent for each query.
    Extracts the results and creates a list of sources in xml format 
    Called during initial report generation and after each reflection
    """
    ...
    results = await asyncio.gather(*[
        process_single_query(query, config, writer, collection)
    ...
    def replace_query_with_source(text, counter):
        parts = text.split('QUERY:')
        result = parts[0]
        for i in range(1, len(parts)):
            result += f'SOURCE {counter} QUERY:' + parts[i]
            counter += 1
        return result, counter

    numbered_citations = []
    for citation in citations:
        numbered_citation, global_source_counter = replace_query_with_source(citation, global_source_counter)
    ...
    search_str = deduplicate_and_format_sources(
        numbered_citations, generated_answers, state_queries
    )
...
async def reflect_on_summary(state: AIRAState, config: RunnableConfig, writer: StreamWriter):
    ...
    # reflect at least once
    if num_reflections is None or num_reflections == 0:
        num_reflections = 1
